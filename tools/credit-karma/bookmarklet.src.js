...
(async () => {
  ...
  try {
    ...
    // Apple provider + a "Deposit" or "Adjustment" description. A manual
    // deposit into Apple Savings would also match (accepted trade-off — rare).
    function isAppleDailyCash(t) {
    ...
    // sign as a reversal. Income clawbacks (very rare) can be corrected
    // manually in EditModal. Apple Daily Cash is handled separately above.
    function isInc(t) { return (t.categoryType || '').toUpperCase() === 'INCOME' || isAppleDailyCash(t); }
    ...
    function naturalAmount(t) {
      ...
      // Other income: always a positive credit.
    ...
    for (const t of all) {
      ...
      rows.push({ date: date, description: t.description || '', amount: naturalAmount(t), category: isAppleDailyCash(t) ? 'Other Income' : mapCat(t.category, t.categoryType), account: acctLabel(t.provider, t.account, t.mask), ck_account: t.account || '', provider: t.provider || '', ck_category: t.category || '', type: isInc(t) ? 'income' : 'expense', account_urn: t.urn || '', last4: t.mask || '', source_id: t.id || '' });
    }
