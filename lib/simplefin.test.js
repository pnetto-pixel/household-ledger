...
describe("mapTransaction — providerDescription (dedup signal preserved when memo wins)", () => {
  it("keeps the generic description/payee alongside the memo-based display description", () => {
    const row = mapTransaction(
      { id: "1", posted: 1700000000, description: "AMAZON.COM*RT4XY1", payee: "Amazon", memo: "100 Pack Mini Sunscreen" },
      { name: "Amazon Card" }
    );
    expect(row.description).toBe("100 Pack Mini Sunscreen");
    expect(row.providerDescription).toBe("AMAZON.COM*RT4XY1");
  });

  it("falls back to payee for providerDescription when description is absent", () => {
    const row = mapTransaction(
      { id: "1", posted: 1700000000, payee: "Amazon", memo: "USB-C cable" },
      { name: "Amazon Card" }
    );
    expect(row.providerDescription).toBe("Amazon");
  });

  it("omits providerDescription when memo did NOT win (nothing additive to carry)", () => {
    const row = mapTransaction(
      { id: "1", posted: 1700000000, description: "Whole Foods", payee: "WFM" },
      { name: "Chase Reserve" }
    );
    expect(row.providerDescription).toBeUndefined();
  });

  it("omits providerDescription when there was no description/payee to preserve", () => {
    const row = mapTransaction({ id: "1", posted: 1700000000, memo: "Order detail" }, { name: "Amazon Card" });
    expect(row.description).toBe("Order detail");
    expect(row.providerDescription).toBeUndefined();
  });
});
