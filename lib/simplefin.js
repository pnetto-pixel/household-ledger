...
// (buildRow/resolveImportCategory default to "Uncategorized" when nothing
// else classifies the row) so a synced row is never silently miscounted as
// Transfer/Income. Must be "Uncategorized", NOT "Other": this placeholder is
// the row's `category` going INTO resolveImportCategory's csvCategory
// resolution (client-side, via classifySimpleFinRows in src/App.jsx) — if it
// were "Other" (a real, user-pickable category), matchOption would treat it
// as a legitimate match and every un-ruled/un-learned SimpleFin row would
// silently land in "Other" instead of surfacing as genuinely unclassified.
