...
// User-configurable equivalent of accountMatchesKeyword, checked against a
// list of patterns (household:*:config's `ignoredSimplefinAccounts`) instead
// of a single hardcoded keyword. Case-insensitive substring match against the
// account's own name/id only — deliberately NOT org name/domain here, since
// the patterns this feeds (Settings → "Ignored SimpleFin accounts") are
// authored against what the user sees for their own accounts, same fields
// `isIgnoredSimplefinAccount` (src/ledger.js) checks client-side. Kept in
// this file (not imported from src/ledger.js) because lib/ is the server
// context and src/ is the client bundle — they must not cross-import.
...
// entirely (no transactions, no holdings, no accountBalances entry, doesn't
// count toward accountCount).
export async function fetchSimplefinTransactions(ignoredPatterns = []) {
  ...
  for (const account of accounts) {
    // Accounts the user chose to ignore (Settings → "Ignored SimpleFin
    // accounts", household:*:config's `ignoredSimplefinAccounts`) are
    // skipped completely — no transactions, no holdings, no accountBalances
    // entry, and they never reach the pending queue or the live fetch.
    if (accountIsIgnored(account, ignoredPatterns)) continue;
    syncedAccountCount++;
